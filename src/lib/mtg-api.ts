...
const MTG_API_BASE =
  process.env.MTG_API_BASE_URL || "https://api.magicthegathering.io/v1";
...
class MTGApiService {
  ...
  private async throttle() {
    ...
    if (timeSinceLastRequest < 750) {
      // Wait at least 750ms between requests
      await new Promise((resolve) =>
        setTimeout(resolve, 750 - timeSinceLastRequest)
      );
    }
  ...
  async getRandomCards(count: number = 10): Promise<MTGCard[]> {
    ...
    try {
      const response = await axios.get(
        `${this.baseUrl}/cards?random=true&pageSize=${Math.min(count, 100)}`
      );
  ...
  async generateBoosterPack(setCode: string): Promise<MTGCard[]> {
    ...
    try {
      const response = await axios.get(
        `${this.baseUrl}/sets/${setCode}/booster`
      );
      return response.data.cards || [];
    } catch (error) {
  ...
  async searchCardsByForeignName(
    name: string,
    language: string
  ): Promise<MTGCard[]> {
...
export const isCardLegal = (card: MTGCard, format: string): boolean => {
  ...
  const legality = card.legalities.find(
    (l) => l.format.toLowerCase() === format.toLowerCase()
  );
...
export const getCardForeignName = (card: MTGCard, language: string): string => {
  ...
  const foreignName = card.foreignNames.find((fn) =>
    fn.language.toLowerCase().includes(language.toLowerCase())
  );
