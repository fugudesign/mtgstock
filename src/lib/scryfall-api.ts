...
class ScryfallApiService {
  ...
  private buildScryfallQuery(params: {
    ...
    if (params.colors) {
      ...
      const colors = params.colors.toLowerCase().split(',').map(c => c.trim())
    ...
    if (params.lang) {
      ...
      const langCode = langMap[params.lang.toLowerCase()] || params.lang.toLowerCase()
  ...
  async getRandomCards(count: number = 10): Promise<MTGCard[]> {
    ...
    for (let i = 0; i < count; i++) {
      await this.throttle()
      try {
        const response = await axios.get<MTGCard>(`${this.baseUrl}/cards/random`)
        cards.push(response.data)
      } catch (error) {
        console.error('Erreur lors de la récupération de carte aléatoire:', error)
      }
    }
  ...
  async autocomplete(query: string): Promise<string[]> {
    ...
    try {
      const response = await axios.get<{ object: string, total_values: number, data: string[] }>(
...
export const isCardLegal = (card: MTGCard, format: string): boolean => {
  const legality = card.legalities[format.toLowerCase() as keyof typeof card.legalities]
