...
export async function POST(
  ...
  try {
    ...
    if (!cardData || !cardData.id) {
      return NextResponse.json({ error: 'Données de carte invalides' }, { status: 400 })
    }
    ...
    if (!isSideboard && currentTotalCards + quantity > DECK_RULES.MAX_CARDS) {
    ...
    const isBasicLand = DECK_RULES.BASIC_LANDS.some(land => 
      cardData.name.includes(land) && cardData.type_line?.includes('Basic Land')
    )

    // Vérifier la carte existe déjà dans le deck
    const existingDeckCard = deck.cards.find(
      c => c.card.scryfallId === cardData.id && c.isSideboard === isSideboard
    )

    if (existingDeckCard) {
      // Vérifier la limite de 4 copies (sauf terrains de base)
      if (!isBasicLand && existingDeckCard.quantity + quantity > DECK_RULES.MAX_COPIES) {
        return NextResponse.json({ 
